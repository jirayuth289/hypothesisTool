...
.controller('mainCtrl', function($scope, Auth, $timeout, $location, $rootScope, $window, $interval, User, AuthToken) {
    var app = this;

    app.loadme = false;
    app.template = 'app/views/template1.html';
    ...
    app.checkSession = function() {
        ...
        if (Auth.isLoggedIn()) {
            ...
            var interval = $interval(function() {
                ...
                //         showModalTimeOut(1); // Open bootstrap modal and let user decide what to do
    ...
    var showModalTimeOut = function(option) {
        ...
        if (option === 1) {
            app.modalHeader = 'คำเตือนกำลังจะหมดเวลาใช้งานเว็บไซต์'; // Set header
            app.modalBody = 'คุณเหลือเวลาใช้งานในระบบ 30  นาที  ต้องการใช้งานต่อหรือไม่?'; // Set body
            ...
            app.modalHeader = 'กำลังออกจากระบบ'; // Set header
            $("#myModal").modal({ backdrop: "static" }); // Open modal
            app.template = 'app/views/template1.html';
    ...
    app.endSession = function() {
        app.choiceMade = true; // Set to true to stop 10-second check in option 1
        hideModalTimeOut(); // Hide modal
        // After 1 second, activate modal option 2 (log out)
        $timeout(function() {
            showModalTimeOut(2); // logout user
        }, 1000);
    };

    // Function to hide the modal
    var hideModalTimeOut = function() {
    ...
    $rootScope.$on('$routeChangeStart', function() {
        ...
        if (Auth.isLoggedIn()) {
            ...
            Auth.getUser().then(function(data) {
                ...
                app.template = 'app/views/template2.html';
            ...
            if (data.data.success) {
                ...
                User.getPermission().then(function(data) {
                    ...
                    if (data.data.permission === 'admin' || data.data.permission === 'user') {
                        $timeout(function() {
                            $location.path('/diagList'); // Redirect to home
    ...
    app.logout = function() {
        showModalTimeOut(2); // Activate modal that logs out user
    };
